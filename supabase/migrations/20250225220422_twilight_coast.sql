/*
  # Fix Username Validation and Constraints

  1. Changes
    - Drop existing constraints and functions in the correct order
    - Add improved username validation
...
DROP TRIGGER IF EXISTS enforce_username_rules_trigger_insert ON user_profiles;

-- Drop existing functions
DROP FUNCTION IF EXISTS enforce_username_rules();

-- Drop existing constraints in the correct order
ALTER TABLE user_profiles 
  DROP CONSTRAINT IF EXISTS username_format CASCADE,
  DROP CONSTRAINT IF EXISTS username_not_empty CASCADE;
